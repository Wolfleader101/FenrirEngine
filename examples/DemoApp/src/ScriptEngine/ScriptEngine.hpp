...
namespace DukTie
{
    ...
    class DukUserType
    {
      public:
        /**
         * @brief Construct a new Duk User-Type object
         *
         * @param ctx duk context to use
         * @param typeName of the object
         */
        DukUserType(duk_context* ctx, const std::string& typeName) : m_ctx(ctx), m_typeName(typeName)
        {
    ...
    {
      public:
        DukFunction(duk_context* ctx, const std::string& funcName) : m_ctx(ctx), m_funcName(funcName)
        {
        }

        void Set(RetType (*funcToCall)(Args...))
        {
            dukglue_register_function(m_ctx, funcToCall, m_funcName.c_str());
        }

        RetType Call(Args... args)
        {
            duk_get_global_string(m_ctx, m_funcName.c_str()); // TODO this might just need to be get_string
            ...
            std::size_t numArgs = 10;
            // sizeof...(args);

            // Call the function with n arguments, and check for errors
            if (duk_pcall(ctx, static_cast<duk_idx_t>(numArgs)) != DUK_EXEC_SUCCESS)
            {
    ...
    class DukContext
    {
      public:
        ...
        template <typename T>
        DukUserType<T> CreateType(const std::string& typeName)
        {
            return DukUserType<T>(m_ctx.get(), typeName);
        }
    ...
    class DukHeap : public DukContext
    {
      public:
        /**
        ...
        DukHeap()
        {
            m_ctx = std::shared_ptr<duk_context>(duk_create_heap(nullptr, nullptr, nullptr, nullptr, nullptr),
                                                 [](duk_context* ctx) {
                                                     // TODO investgiate why this is causing a crash
                                                     //  if (ctx != nullptr)
...
namespace Fenrir
{
    ...
    class ScriptEngine
    {
      ...
      private:
        DukTie::DukHeap m_heap; //! the global context
